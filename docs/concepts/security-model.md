...
- The [browser-open kit](../kits/browser-open.md) (on by default) runs a small **host broker** for the container's lifetime that opens `http(s)` URLs from the container in your host browser. The broker is not reachable beyond the host — a Unix socket mounted only into that container on native Linux, or a `127.0.0.1` loopback bind on Docker Desktop/macOS — with a per-container secret token as defense-in-depth. Opening an `http(s)` URL is a capability every sandboxed app already has; disable the kit if you don't want it. Other schemes are refused unless you allowlist them one by one under `kits.browser-open.schemes` — that opt-in lets the container launch the host app registered for that scheme with an argument of its choosing, so grant only the schemes you actually want (a scheme like `file` would expose host files to host apps). For OAuth logins, it can also briefly bridge a loopback callback port from the host into the container: the bridge binds host loopback only, forwards into the agent's own container (it cannot reach host services — a taken port just fails), and expires after five minutes.
