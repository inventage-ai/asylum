...
- **Kits**: merged per-key (a project kit entry deep-merges with the global one: `packages`/`build`/`schemes` accumulate across layers, other options are last-wins). The `--kits` CLI flag is the exception: it replaces the kit map entirely.
